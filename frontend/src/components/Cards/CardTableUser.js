...
// components
class CardTableUser extends Component {
  state = {
    users: [],
    total: 0,
    countPerPage: 10,
  ...
  getUsers = () => {
    ...
    if (user !== null) {
      axios.get(`${direction.API_USER}?page=${page}&per_page=${countPerPage}`, {
        headers: {
          'x-auth-token': user.accessToken
        }
      }).then(
        response => {
          this.setState({ users: response.data.data, total: response.data.total, loading: true });
        },
        error => {
  ...
  delete = (id) => {
    const user = JSON.parse(localStorage.getItem('user')) || null;
    if (user !== null) {
      axios.delete(direction.API_USER + `${id}`, {
        headers: {
          'x-auth-token': user.accessToken
        }
      }).then(res => {
      ...
      let log = {
        message: `Ha eliminado un usuario con el id:  ${id}`,
        userId: user.id,
  ...
  render() {
    ...
    const column = [
      ...
      {
        name: "Role",
        cell: row => row.roleId.name
      },
      {
        name: "Fecha de último acceso",
      ...
      },
      {
        ...
        cell: row => <React.Fragment>
          <button className="bg-lightBlue-500 text-white active:bg-lightBlue-600 font-bold uppercase text-xs px-4 py-2 rounded shadow hover:shadow-md outline-none focus:outline-none mr-1 mb-1 ease-linear transition-all duration-150" type="button"
            onClick={() => this.renderUsers(row.id)}
          >
    ...
    const { users, total, countPerPage } = this.state;
...
export default connect(mapStateToProps, mapDispatchToProps)(CardTableUser);
